# Subscription Plugin for Mollie
...
The Subscription Plugin for Mollie enables you to offer a complete subscription system in your Shopware shop. Your customers can order products as recurring deliveries and benefit from automatic discounts. Renewals are processed automatically in the background — including payment processing via Mollie (credit card, SEPA direct debit, PayPal) or manual payment methods (invoice, prepayment).
...
- **Subscription Discounts**: Percentage discounts on subscription orders, plus optional loyalty tiers based on number of renewals (database-only, no admin UI)
- **Mollie Payment Integration**: Recurring payments via credit card, SEPA direct debit and PayPal with automatic mandate management
...
- **Admin Dashboard**: Key metrics for active subscriptions, monthly recurring revenue, upcoming renewals, and churn rate
...
- **Free Items**: Add free products to renewals (via the admin API, no admin UI)
- **Change Log**: Complete audit trail of all subscription changes, stored in the database

## Requirements

- Shopware Version: 6.6.0 or higher (up to 6.7.x)
- PHP Version: 8.2 or higher (required by Shopware 6.6+)
- Mollie API client: bundled with the plugin, no separate installation required
- Mollie Account: Required for credit card, SEPA direct debit and PayPal
...
3. Configure basic settings in **Extensions → My Extensions → Subscription Plugin for Mollie → Configure**
4. Enter your Mollie API keys in the plugin configuration
5. Configure your desired intervals and discounts
6. Enable subscription options on desired products under **Catalogues → Products → [Product] → General**, in the **Subscription Options** card at the bottom of the page
