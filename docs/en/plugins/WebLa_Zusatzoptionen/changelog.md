...
- Options with quantity selection can be marked as **Flat rate**: the price is charged once per unit of the main product, no matter which quantity the customer selects. The storefront shows a "Flat rate" hint below the option price. The setting is available in the option dialog and per option in the seeder.
- Options now have a **Minimum quantity** (default 1) and **Maximum quantity** (default 100). The storefront quantity field respects the limits, and quantities outside the limits are corrected when the option is added to the cart or changed there. Both values can also be set per option in the seeder.
...
- New setting "Hide option sets of deactivated seeders": when enabled, the storefront no longer shows option sets whose seeder is deactivated. The option sets stay in place, so reactivating the seeder restores them unchanged. Off by default.
- New setting "Delete option sets when a seeder is deleted": when disabled, the option sets generated by a seeder remain on the products after the seeder is removed. On by default, matching the previous behaviour.

**Bug Fixes**

- The seeder now carries the "Preselected" flag into the options it creates.
- Option set state written by the seeder is now saved through the data layer, so caches and indexes are refreshed correctly.
